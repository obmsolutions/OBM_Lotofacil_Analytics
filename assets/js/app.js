(function () {
    ...
    function bindControls() {
        ...
        document.getElementById('exportCheckResultsButton').addEventListener('click', exportCheckResults);
        document.getElementById('drawInput').addEventListener('input', invalidateCheckResults);
    ...
    function checkGames() {
        try {
            ...
            state.lastCheck = { draw, results, counts };
            document.getElementById('exportCheckResultsButton').disabled = false;
            ...
            invalidateCheckResults(false);
            showAlert(error.message, 'error');
        }
    }

    function invalidateCheckResults(clearDisplay = true) {
        state.lastCheck = null;
        document.getElementById('exportCheckResultsButton').disabled = true;
        if (clearDisplay) {
            document.getElementById('checkSummary').innerHTML = '';
            document.getElementById('checkDetails').innerHTML = '';
        }
    }

    function exportCheckResults() {
        if (!state.lastCheck) return showAlert('Faça a conferência dos jogos antes de baixar o relatório.', 'error');
        const content = OBMLotofacilCore.buildCheckReport(
            state.lastCheck.draw,
            state.lastCheck.results,
            new Date().toLocaleString('pt-BR')
        );
        downloadText(`obm-lotofacil-resultado-conferencia-${dateStamp()}.txt`, content);
        showAlert('Relatório de resultados baixado com sucesso.');
    }
